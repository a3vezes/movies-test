...
RSpec.describe Movies::API do
  ...
  describe 'GET /api/v1/movies/:id' do
    ...
    it 'returns a validation error when the movie doesnt exists' do
      get '/api/v1/movies/-1'
  ...
  describe 'Movie Rating' do
    let!(:rating_movies) { Movie.create(title: 'rating', release_date: Date.tomorrow) }

    it 'calculates rates correctly' do
      Rating.create(movie_id: rating_movies[:id], grade: 4)
      Rating.create(movie_id: rating_movies[:id], grade: 2)

      get "/api/v1/movies/#{rating_movies.id}"

      expect(response.body).to include('3.0')
    end

    it 'calculates rates correctly with two decimals' do
      Rating.create(movie_id: rating_movies[:id], grade: 5)
      Rating.create(movie_id: rating_movies[:id], grade: 3)
      Rating.create(movie_id: rating_movies[:id], grade: 2)

      get "/api/v1/movies/#{rating_movies.id}"

      expect(response.body).to include('3.33')
    end
  end
end
